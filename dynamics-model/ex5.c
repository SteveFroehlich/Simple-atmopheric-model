...
PetscErrorCode FormInitialSolution(DM da,Vec Xglobal,void *ctx)
{
  ...
  double half = (ys + ym)/2;
  //high and low pressure with zonal zero edge Grad(T) at mid domain
  else if (user->init == 2) {
    for (j=ys; j<ys+ym; j++) {
      for (i=xs; i<xs+xm; i++) {
        if (j > half) X[j][i] = user->Ts + 2;
        else  X[j][i].Ts = user->Ts;
    ...
    // initialize high pressure perturbation in the center of the domain
    for (j=ys; j<ys+ym; j++) {
      for (i=xs; i<xs+xm; i++) {
          X[j][i].Ts = user->Ts - i*0.0001;
          X[j][i].Ta = X[j][i].Ts - 5;
          ...
          X[j][i].p  = 1.25;
             ...
             if (pow((((i-x/4)*(i-x/4))+((j-y/4)*(j-y/4))),0.5) <= r)
                   X[j][i].p   += 0.00001;

             if (pow((((i-   ((x/4)*3)   )*(i-    ((x/4)*3)   ))+((j-   ((y/2)*1)   )*(j-   $
                   X[j][i].p   -= 0.00001;
          }
      }
    }
